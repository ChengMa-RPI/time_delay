...
def draw_brace(ax, xspan, color, linewidth, text):
...
def plot_xs_weight(ax, xlabel, ylabel, A, m, weight, xs, color, linewidth, alpha, color_bias, title_letter, xs_multi=None, A_unit=None, group_index=None):
    ...
    if xs_multi is not None:
        ...
        for i, xs_multi_i in enumerate(xs_multi):
            ...
            x_eff[i] = x_eff_i
        ax.plot(weight, x_eff, linewidth=linewidth*0.8, alpha=0.3, color='tab:grey')
...
def plot_reduced_network_topology(ax, A_reduced, m, color, color_bias, node_size_bias, title_letter):
    """TODO: Docstring for plot_network_topology.
...
def main_fig(network_type_list, space_list, N_list, d_list, seed_list, weight_list, dynamics):
    ...
    ax.annotate( 'One-dimension Reduction', xy = (1.2, 0.8),  xytext=(0.55, 0.7), xycoords='axes fraction', fontsize=14, color='tab:grey', weight='bold')
    ...
    for (i, network_type), N, d, seed, space in zip(enumerate(network_type_list), N_list, d_list, seed_list, space_list):
        ...
        linewidth = 2
        alpha = 0.8
        plot_xs_weight(ax, xlabel, ylabel, A_unit, len(A_unit), weight_list, xs_multi, colors[i], linewidth, alpha, color_bias, title_letter)
        if i == 0:
        ...
        if i == 0:
            ax.annotate('Topology', xy = (-0.7, 0.5),  xytext=( 0.25, 1.4), xycoords='axes fraction', fontsize=15, color='tab:grey', alpha=.5, weight='bold')
        plot_reduced_network_topology(ax, A_reduced, m, colors[i], color_bias, node_size_bias, title_letter)
        ...
        for j, m in enumerate([3, 10, 20]):
            ...
            if i == 0:
                ...
                ax.annotate('$\\beta_{ij}$', xy = (-0.7, 0.5),  xytext=(0.8, 0.8), xycoords='axes fraction', fontsize=15, color='k', alpha=.8, weight='bold')
            xs_group = read_xs(network_type, N, space, d, seed, m, weight_list)
            group_index = group_index_from_feature_Kmeans(feature, m)
    ...
    ax.set_axis_off()
    ax.annotate( 'm-dimension Reduction', xy = (0.7, 0.5),  xytext=(0.23, 0.5), xycoords='axes fraction', fontsize=14, color='tab:grey', weight='bold')
    dxdt = r'$\frac{dx_i}{dt} = F(x_i) + w \sum_{j=1}^{N} \beta_{ij} G(x_i, x_j)$'
    t = ax.text(0.61, 0.58, dxdt, ha="center", va="center", rotation=0, size=15, bbox=dict(boxstyle="round,pad=0.3", fc="tab:grey", ec="k", lw=1, alpha=0.5))
